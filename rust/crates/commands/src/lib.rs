...
const SLASH_COMMAND_SPECS: &[SlashCommandSpec] = &[
    ...
    SlashCommandSpec {
        name: "llm",
        aliases: &["models"],
        summary: "Manage LLM profiles, API keys, and turn token limits",
        argument_hint: Some("[list|add|add-env|use|remove|clear-active|limits|help]"),
        resume_supported: false,
    },
    SlashCommandSpec {
        name: "thread",
        aliases: &["threads"],
        summary: "Manage folder-scoped conversation threads",
        argument_hint: Some("[list|add <name> <folder>|switch <name>|remove <name>|help]"),
        resume_supported: false,
    },
    SlashCommandSpec {
        name: "app",
        aliases: &["apps"],
        summary: "Register reusable app workflows with descriptions",
        argument_hint: Some("[list|add <name> <command>|run <name>|remove <name>|help]"),
        resume_supported: false,
    },
    SlashCommandSpec {
        name: "doctor",
...
pub fn validate_slash_command_input(
    ...
    Ok(Some(match command {
        ...
        "llm" | "models" => SlashCommand::Llm { args: remainder },
        "thread" | "threads" => SlashCommand::Thread { args: remainder },
        "app" | "apps" => SlashCommand::App { args: remainder },
...
fn slash_command_category(name: &str) -> &'static str {
    match name {
        ...
        | "approve" | "deny" | "allowed-tools" | "add-dir" | "app" => "Tools",
...
mod tests {
    ...
    fn parses_supported_slash_commands() {
        ...
        assert_eq!(
            SlashCommand::parse("/llm list"),
            Ok(Some(SlashCommand::Llm {
                args: Some("list".to_string())
            }))
        );
        assert_eq!(
            SlashCommand::parse("/thread switch backend"),
            Ok(Some(SlashCommand::Thread {
                args: Some("switch backend".to_string())
            }))
        );
        assert_eq!(
            SlashCommand::parse("/app run review"),
            Ok(Some(SlashCommand::App {
                args: Some("run review".to_string())
            }))
        );
        assert_eq!(
            SlashCommand::parse("/workspace ../repo"),
