...
use runtime::{
    check_freshness, dedupe_superseded_commit_events, detect_interactive_shell_command, edit_file,
    execute_bash, glob_search, grep_search, interactive_blocked_output, load_system_prompt,
    lsp_client::LspRegistry,
    mcp_tool_bridge::McpToolRegistry,
    permission_enforcer::{EnforcementResult, PermissionEnforcer},
    read_file, resolve_shell_timeout_ms,
...
fn resolve_repl_runtime(language: &str) -> Result<ReplRuntime, String> {
    match language.trim().to_ascii_lowercase().as_str() {
        "python" | "py" => {
            #[cfg(windows)]
            {
                // Windows 上经常只有 py launcher，没有 python/python3 可执行文件。
...
fn detect_powershell_shell() -> std::io::Result<String> {
    // Windows 下 PATH 里常见的是 `pwsh.cmd`/`powershell.exe`。
    // 这里解析真实路径，避免 `Command::new("pwsh")` 在测试 stub
    // 或某些 PATH 配置下无法命中对应脚本。
...
fn execute_shell_command(
    ...
    if let Some(block) = detect_interactive_shell_command(command, &cwd) {
    ...
    let timeout_ms = resolve_shell_timeout_ms(timeout);
    ...
    loop {
        ...
        if started.elapsed() >= Duration::from_millis(timeout_ms) {
            ...
            return Ok(timeout_output(timeout_ms, stdout, stderr, None, None));
        }
...
mod tests {
    ...
    use super::{
        ...
        GlobalToolRegistry, ProviderRuntimeClient, SubagentToolExecutor,
    };
    ...
    fn shell_stdout_command(text: &str) -> String {
        if cfg!(windows) {
            format!("echo|set /p={text}")
        } else {
            format!("printf '{text}'")
        }
    }

    fn shell_stderr_and_exit_command(text: &str, code: i32) -> String {
        if cfg!(windows) {
            format!("echo {text} 1>&2 & exit /b {code}")
        } else {
            format!("printf '{text}' >&2; exit {code}")
        }
    }
    ...
    fn shell_interactive_command() -> String {
        if cfg!(windows) {
            "echo before & pause".to_string()
        } else {
            "printf 'before'; read answer".to_string()
        }
    }
    ...
    fn python_smoke_timeout_ms() -> u64 {
        if cfg!(windows) {
            // Windows 在整仓测试并发运行时，首次拉起 Python 进程会明显更慢。
            // 这里放宽 smoke test 超时，避免把环境冷启动误判成 REPL 功能失败。
            2_000
        } else {
            500
        }
    ...
    fn powershell_stub_name() -> &'static str {
        if cfg!(windows) {
            "pwsh.cmd"
        } else {
            "pwsh"
    ...
    fn mark_test_script_executable(path: &Path) {
        #[cfg(unix)]
        {
    ...
    fn worker_create_merges_config_trusted_roots_without_per_call_override() {
        ...
        let settings = serde_json::to_string(&json!({ "trustedRoots": [tmp_root] }))
            .expect("trustedRoots settings should serialize");
    ...
    fn skill_loads_local_skill_prompt() {
        ...
        assert!(
            normalize_display_path(output["path"].as_str().expect("path"))
        ...
        assert!(
            normalize_display_path(dollar_output["path"].as_str().expect("path"))
                .ends_with("/help/SKILL.md")
        );
    ...
    fn skill_resolves_project_local_skills_and_legacy_commands() {
        ...
        assert!(
            normalize_display_path(skill_output["path"].as_str().expect("path"))
        ...
        assert!(
            normalize_display_path(command_output["path"].as_str().expect("path"))
    ...
    fn skill_loads_project_local_omc_and_agents_skill_prompts() {
        ...
        assert!(
            normalize_display_path(omc_output["path"].as_str().expect("path"))
                .ends_with(".omc/skills/hud/SKILL.md")
        );
        assert_eq!(omc_output["description"], "Project-local OMC HUD helper");
        assert!(
            normalize_display_path(agents_output["path"].as_str().expect("path"))
    ...
    fn skill_loads_direct_skill_and_legacy_command_from_claude_config_dir() {
        ...
        assert!(
            normalize_display_path(direct_skill_output["path"].as_str().expect("path"))
        ...
        assert!(
            normalize_display_path(legacy_command_output["path"].as_str().expect("path"))
  ...
  "nbformat_minor": 5
    ...
    fn bash_tool_reports_success_exit_failure_timeout_and_background() {
        ...
        let failure = execute_tool(
            "bash",
            &json!({ "command": shell_stderr_and_exit_command("oops", 7) }),
        )
        ...
        let timeout = execute_tool(
            "bash",
            &json!({ "command": shell_sleep_command(1), "timeout": 10 }),
        )
        ...
        let background = execute_tool(
            "bash",
            &json!({ "command": shell_sleep_command(1), "run_in_background": true }),
        )
    ...
    #[test]
    fn bash_tool_blocks_interactive_commands_before_execution() {
        let blocked = execute_tool("bash", &json!({ "command": shell_interactive_command() }))
            .expect("interactive bash should return structured output");
        let blocked_output: serde_json::Value = serde_json::from_str(&blocked).expect("json");
        assert_eq!(
            blocked_output["returnCodeInterpretation"],
            "interactive_blocked"
        );
        assert_eq!(
            blocked_output["structuredContent"][0]["kind"],
            "requires_terminal_handoff"
        );
    }
    ...
    fn glob_and_grep_tools_cover_success_and_errors() {
        ...
        assert!(
            normalize_display_path(globbed_output["filenames"][0].as_str().expect("filename"))
    ...
    fn repl_executes_python_code() {
        let result = execute_tool(
            ...
            &json!({
                "language": "python",
                "code": "print(1 + 1)",
                "timeout_ms": python_smoke_timeout_ms()
            }),
        );
        if matches!(&result, Err(error) if error.contains("python runtime not found")) {
            return;
        }
        let result = result.expect("REPL should succeed");
        let output: serde_json::Value = serde_json::from_str(&result).expect("json");
    ...
    fn powershell_runs_via_stub_shell() {
        ...
        let script = dir.join(powershell_stub_name());
        std::fs::write(
            &script,
            if cfg!(windows) {
...
"#
        ...
        .expect("write script");
        mark_test_script_executable(&script);
        let original_path = std::env::var("PATH").unwrap_or_default();
        std::env::set_var(
            "PATH",
            format!("{}{}{}", dir.display(), path_env_separator(), original_path),
        );
    ...
    #[test]
    fn powershell_blocks_interactive_commands_before_execution() {
        let blocked = execute_tool(
            "PowerShell",
            &json!({"command": "Write-Output before; Read-Host 'name'"}),
        )
        .expect("interactive PowerShell should return structured output");
        let blocked_output: serde_json::Value = serde_json::from_str(&blocked).expect("json");
        assert_eq!(
            blocked_output["returnCodeInterpretation"],
            "interactive_blocked"
    ...
    fn given_no_enforcer_when_bash_then_executes_normally() {
        ...
        let result = registry
            .execute("bash", &json!({ "command": shell_stdout_command("ok") }))
            .expect("bash should succeed without enforcer");
        let output: serde_json::Value = serde_json::from_str(&result).expect("json");
        assert_eq!(output["stdout"].as_str().expect("stdout").trim(), "ok");
    }
