...
use crate::bash::{detect_interactive_shell_command, resolve_shell_timeout_ms};
...
impl HookRunner {
    ...
    fn run_command(
        ...
        if let Some(block) = detect_interactive_shell_command(command, &cwd) {
            return HookCommandOutcome::Failed {
                parsed: ParsedHookOutput {
                    messages: vec![format!(
                        "{} hook `{command}` requires terminal handoff: {}",
                        event.as_str(),
                        block.reason
                    )],
        ...
        match child.output_with_stdin(
            payload.as_bytes(),
            abort_signal,
            resolve_shell_timeout_ms(Some(DEFAULT_HOOK_TIMEOUT_MS)),
        ) {
            ...
            Ok(CommandExecution::TimedOut { timeout_ms }) => HookCommandOutcome::Failed {
                parsed: ParsedHookOutput {
                    messages: vec![format!(
                        "{} hook `{command}` timed out after {timeout_ms} ms while handling `{tool_name}`",
                        event.as_str()
                    )],
                    ..ParsedHookOutput::default()
                },
    ...
    fn output_with_stdin(
        ...
        loop {
            ...
            if started.elapsed() >= Duration::from_millis(timeout_ms) {
                let _ = child.kill();
                let _ = child.wait_with_output();
                return Ok(CommandExecution::TimedOut { timeout_ms });
            }
...
mod tests {
    ...
    fn executes_hooks_in_configured_order() {
        // given
        let first = shell_snippet("printf 'first'");
        let second = shell_snippet("printf 'second'");
        let runner = HookRunner::new(RuntimeHookConfig::new(
    ...
    #[test]
    fn blocks_interactive_hooks_before_execution() {
        let interactive = if cfg!(windows) {
            "echo before & pause"
        } else {
            "printf 'before'; read answer"
        };
        let runner = HookRunner::new(RuntimeHookConfig::new(
        ...
        let result = runner.run_pre_tool_use("bash", r#"{"command":"echo hi"}"#);

        assert!(result.is_failed());
        assert!(result
            .messages()
            .iter()
            .any(|message| message.contains("terminal handoff")));
    }
