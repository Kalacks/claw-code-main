...
// Windows 上首次拉起 Python MCP 测试进程明显更慢，过小的测试超时会把
// 启动抖动误判成协议失败。
const MCP_INITIALIZE_TIMEOUT_MS: u64 = 1_500;
...
#[cfg(all(test, windows))]
const MCP_LIST_TOOLS_TIMEOUT_MS: u64 = 1_500;
#[cfg(all(test, not(windows)))]
...
mod tests {
    ...
    fn write_echo_script() -> PathBuf {
        ...
        .expect("write script");
        mark_script_executable(&script_path);
        script_path
    }
    ...
    fn given_child_exits_after_discovery_when_calling_twice_then_second_call_succeeds_after_reset()
    {
        ...
        runtime.block_on(async {
            ...
            assert!(
                lines.len() >= 4,
                "expected reconnect sequence, got {lines:?}"
            );
            assert!(lines.contains(&"tools/list"));
            assert!(lines.ends_with(&["initialize", "tools/call"]));
            assert!(
                lines.iter().filter(|line| **line == "initialize").count() >= 2,
                "expected at least two initialize calls after reset, got {lines:?}"
            );
            ...
            let servers = BTreeMap::from([
                ...
                (
                    ...
                    ScopedMcpServerConfig {
                        ...
                        config: McpServerConfig::Stdio(McpStdioServerConfig {
                            command: python_command(),
                            args: vec![broken_script_path.to_string_lossy().into_owned()],
                            env: BTreeMap::new(),
                            tool_call_timeout_ms: None,
                        }),
                    },
