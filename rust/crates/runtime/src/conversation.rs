...
const DEFAULT_AUTO_COMPACTION_TEXT_CHARS_THRESHOLD: usize = 256 * 1024;
const AUTO_COMPACTION_TEXT_CHARS_ENV_VAR: &str = "CLAUDE_CODE_AUTO_COMPACT_TEXT_CHARS";
...
pub struct ConversationRuntime<C, T> {
    ...
    auto_compaction_text_chars_threshold: usize,
    ...
    T: ToolExecutor,
{
    ...
    pub fn new_with_features(
        ...
        Self {
            ...
            auto_compaction_text_chars_threshold: auto_compaction_text_chars_threshold_from_env(),
    ...
    #[must_use]
    pub fn with_auto_compaction_text_chars_threshold(mut self, threshold: usize) -> Self {
        self.auto_compaction_text_chars_threshold = threshold;
        self
    }
    ...
    pub fn run_turn(
        ...
        let mut auto_compaction_removed_count = 0usize;

        loop {
            auto_compaction_removed_count += self.maybe_auto_compact();
            iterations += 1;
        ...
        auto_compaction_removed_count += self.maybe_auto_compact();
        let auto_compaction = (auto_compaction_removed_count > 0).then_some(AutoCompactionEvent {
            removed_message_count: auto_compaction_removed_count,
        });
    ...
    fn maybe_auto_compact(&mut self) -> usize {
        let exceeded_text_limit =
            estimate_session_text_chars(&self.session) >= self.auto_compaction_text_chars_threshold;
        let estimated_session_input_tokens = estimate_session_tokens(&self.session);
        let token_threshold =
            usize::try_from(self.auto_compaction_input_tokens_threshold).unwrap_or(usize::MAX);
        let exceeded_estimated_input_limit = estimated_session_input_tokens >= token_threshold;
        let exceeded_usage_input_limit = self.usage_tracker.cumulative_usage().input_tokens
            >= self.auto_compaction_input_tokens_threshold;
        // 中文注释：同一轮里命中任一阈值只压缩一次，避免重复压缩导致上下文过度裁剪。
        if exceeded_text_limit || exceeded_estimated_input_limit || exceeded_usage_input_limit {
            self.apply_auto_compaction()
        } else {
            0
        }
    }

    fn apply_auto_compaction(&mut self) -> usize {
        // 中文注释：自动压缩保留最小必要尾部，防止“消息条数多但单条不大”时压不下来。
        let result = compact_session(&self.session, auto_compaction_config());
...
#[must_use]
pub fn auto_compaction_text_chars_threshold_from_env() -> usize {
    parse_auto_compaction_text_chars_threshold(
        std::env::var(AUTO_COMPACTION_TEXT_CHARS_ENV_VAR)
            .ok()
            .as_deref(),
    )
}
...
#[must_use]
fn parse_auto_compaction_text_chars_threshold(value: Option<&str>) -> usize {
    value
        .and_then(|raw| raw.trim().parse::<usize>().ok())
        .filter(|threshold| *threshold > 0)
        .unwrap_or(DEFAULT_AUTO_COMPACTION_TEXT_CHARS_THRESHOLD)
}

#[must_use]
fn estimate_session_text_chars(session: &Session) -> usize {
    // 中文注释：估算文本体量时跳过压缩摘要系统头，只统计真实会话正文。
    let start = usize::from(session.compaction.is_some());
    session
        .messages
        .iter()
        .skip(start)
        .map(estimate_message_text_chars)
        .sum()
}

#[must_use]
fn estimate_message_text_chars(message: &ConversationMessage) -> usize {
    message
        .blocks
        .iter()
        .map(|block| match block {
            ...
            ContentBlock::ToolUse { id, name, input } => {
                id.chars().count() + name.chars().count() + input.chars().count()
            }
            ContentBlock::ToolResult {
                ...
                output,
                ..
            } => tool_use_id.chars().count() + tool_name.chars().count() + output.chars().count(),
        })
        .sum()
}

fn auto_compaction_config() -> CompactionConfig {
    CompactionConfig {
        preserve_recent_messages: 4,
        preserve_recent_budget_tokens: 0,
...
mod tests {
    use super::{
        build_assistant_message, parse_auto_compaction_text_chars_threshold,
        ...
        DEFAULT_AUTO_COMPACTION_TEXT_CHARS_THRESHOLD,
    };
    ...
    fn shell_snippet(script: &str) -> String {
        ...
        if let Some(json) = script.strip_prefix("printf '%s' '") {
            if let Some(json) = json.strip_suffix('\'') {
                return format!(
                    "powershell -NoProfile -Command Write-Output '{}'",
                    escape_single_quoted(json)
                );
            }
        }

        if let Some(message_with_tail) = script.strip_prefix("printf '") {
            if let Some((message, exit_code)) = message_with_tail.split_once("'; exit ") {
                return format!(
                    "powershell -NoProfile -Command Write-Output '{}'; exit {}",
                    escape_single_quoted(message),
                    exit_code.trim()
                );
            }

            if let Some(message) = message_with_tail.strip_suffix('\'') {
                return format!(
                    "powershell -NoProfile -Command Write-Output '{}'",
                    escape_single_quoted(message)
                );
    ...
    #[test]
    fn auto_compaction_text_chars_threshold_defaults_and_parses_values() {
        assert_eq!(
            parse_auto_compaction_text_chars_threshold(None),
            DEFAULT_AUTO_COMPACTION_TEXT_CHARS_THRESHOLD
        );
        assert_eq!(
            parse_auto_compaction_text_chars_threshold(Some("262144")),
            262_144
        );
        assert_eq!(
            parse_auto_compaction_text_chars_threshold(Some("0")),
            DEFAULT_AUTO_COMPACTION_TEXT_CHARS_THRESHOLD
        );
        assert_eq!(
            parse_auto_compaction_text_chars_threshold(Some("not-a-number")),
            DEFAULT_AUTO_COMPACTION_TEXT_CHARS_THRESHOLD
        );
    }

    #[test]
    fn auto_compacts_when_text_chars_threshold_is_crossed() {
        struct SimpleApi;
        impl ApiClient for SimpleApi {
            fn stream(
                &mut self,
        ...
        let mut session = Session::new();
        session.messages = vec![
            crate::session::ConversationMessage::user_text("A".repeat(80)),
            crate::session::ConversationMessage::assistant(vec![ContentBlock::Text {
                text: "B".repeat(80),
            }]),
            crate::session::ConversationMessage::user_text("C".repeat(80)),
            crate::session::ConversationMessage::assistant(vec![ContentBlock::Text {
                text: "D".repeat(80),
            }]),
            crate::session::ConversationMessage::user_text("E".repeat(80)),
        ];
        ...
        .with_auto_compaction_text_chars_threshold(200);

        let summary = runtime
            .run_turn("trigger text compaction", None)
            .expect("turn should succeed");
    ...
    #[test]
    fn auto_compacts_when_estimated_session_tokens_cross_threshold() {
        struct SimpleApi;
        impl ApiClient for SimpleApi {
            fn stream(
                &mut self,
        ...
        let mut session = Session::new();
        session.messages = vec![
            crate::session::ConversationMessage::user_text("X".repeat(900)),
            crate::session::ConversationMessage::assistant(vec![ContentBlock::Text {
                text: "Y".repeat(900),
            }]),
            crate::session::ConversationMessage::user_text("Z".repeat(900)),
            crate::session::ConversationMessage::assistant(vec![ContentBlock::Text {
                text: "W".repeat(900),
            }]),
            crate::session::ConversationMessage::user_text("Q".repeat(900)),
        ];
        ...
        .with_auto_compaction_input_tokens_threshold(200)
        .with_auto_compaction_text_chars_threshold(usize::MAX);

        let summary = runtime
            .run_turn("trigger token compaction", None)
            .expect("turn should succeed");
