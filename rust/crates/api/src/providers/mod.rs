...
pub fn resolve_model_alias(model: &str) -> String {
    let trimmed = strip_provider_namespace(model.trim());
...
fn strip_provider_namespace(model: &str) -> &str {
    let Some((provider, namespaced_model)) = model.split_once('/') else {
        return model;
    };
    if namespaced_model.trim().is_empty() {
        return model;
    }
    match provider.to_ascii_lowercase().as_str() {
        "anthropic" | "openai" | "xai" | "deepseek" => namespaced_model.trim(),
        _ => model,
    }
}
...
mod tests {
    ...
    #[test]
    fn namespaced_models_route_to_their_provider_and_strip_the_namespace() {
        ...
        assert_eq!(resolve_model_alias("openai/gpt-4.1-mini"), "gpt-4.1-mini");

        let deepseek_kind = super::metadata_for_model("deepseek/deepseek-chat").map_or_else(
            || detect_provider_kind("deepseek/deepseek-chat"),
            |m| m.provider,
        );
        assert_eq!(deepseek_kind, ProviderKind::DeepSeek);
        assert_eq!(
    ...
    fn returns_context_window_metadata_for_supported_models() {
        ...
        assert_eq!(
            model_token_limit("deepseek")
                .expect("deepseek should resolve to a registered model")
                .context_window_tokens,
            128_000
        );
    }
