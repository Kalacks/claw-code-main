...
impl StreamState {
    ...
    fn ingest_chunk(&mut self, chunk: ChatCompletionChunk) -> Result<Vec<StreamEvent>, ApiError> {
        ...
        if let Some(usage) = chunk.usage {
            self.usage = Some(usage.normalized_usage());
        }
...
impl OpenAiUsage {
    fn normalized_usage(&self) -> Usage {
        let has_cache_breakdown =
            self.prompt_cache_hit_tokens > 0 || self.prompt_cache_miss_tokens > 0;
        Usage {
            input_tokens: if has_cache_breakdown {
                self.prompt_cache_miss_tokens
            } else {
                self.prompt_tokens
            },
            cache_creation_input_tokens: 0,
            cache_read_input_tokens: if has_cache_breakdown {
    ...
    match message.role.as_str() {
        "assistant" => {
            ...
            if text.is_empty() && tool_calls.is_empty() {
                ...
                translated.insert("role".to_string(), json!("assistant"));
                translated.insert(
                    "content".to_string(),
                    json!((!text.is_empty()).then_some(text)),
                );
                if !tool_calls.is_empty() {
                    translated.insert("tool_calls".to_string(), Value::Array(tool_calls));
                }
                vec![Value::Object(translated)]
            }
        }
        _ => message
            .content
...
fn openai_tool_definition(tool: &ToolDefinition) -> Value {
    ...
    let mut tool_definition = serde_json::Map::new();
    tool_definition.insert("type".to_string(), json!("function"));
    tool_definition.insert("function".to_string(), Value::Object(function));
    Value::Object(tool_definition)
}
...
fn normalize_response(
    ...
    Ok(MessageResponse {
        ...
        usage: response
            .usage
            .as_ref()
            .map_or_else(Usage::default, OpenAiUsage::normalized_usage),
        request_id: None,
    })
}
...
mod tests {
    ...
    fn assistant_text_history_omits_empty_tool_calls() {
        let payload = build_chat_completion_request(
            &MessageRequest {
                ...
                messages: vec![
                    ...
                    InputMessage::user_text("which api are you using"),
                ],
                stream: false,
                ..Default::default()
            },
        ...
        assert_eq!(payload["messages"][1]["content"], json!("hi there"));
        assert!(
            payload["messages"][1].get("tool_calls").is_none(),
            "assistant text-only history should not send an empty tool_calls array"
        );
        assert_eq!(payload["messages"][2]["role"], json!("user"));
    }

    #[test]
    fn assistant_tool_history_keeps_tool_calls() {
        let payload = build_chat_completion_request(
            &MessageRequest {
                model: "deepseek-chat".to_string(),
                max_tokens: 64,
    ...
    #[test]
    fn tool_result_history_omits_nonstandard_is_error_field() {
        let payload = build_chat_completion_request(
            &MessageRequest {
                model: "deepseek-chat".to_string(),
                max_tokens: 64,
        ...
        assert_eq!(payload["messages"][0]["content"], json!("ok"));
        assert!(
            payload["messages"][0].get("is_error").is_none(),
            "OpenAI-compatible tool messages should not include nonstandard is_error"
        );
    }
    ...
    #[test]
    fn tool_definition_omits_null_description() {
        let payload = build_chat_completion_request(
            &MessageRequest {
                model: "deepseek-chat".to_string(),
                max_tokens: 64,
        ...
        assert_eq!(payload["tools"][0]["type"], json!("function"));
        assert_eq!(payload["tools"][0]["function"]["name"], json!("weather"));
        assert_eq!(
            payload["tools"][0]["function"]["parameters"],
            json!({"type": "object"})
        );
        assert!(payload["tools"][0]["function"].get("description").is_none());
    }

    #[test]
    fn deepseek_usage_maps_cache_hit_and_miss_tokens() {
    ...
    #[test]
    fn openai_usage_without_cache_breakdown_falls_back_to_prompt_tokens() {
        let usage = OpenAiUsage {
            prompt_tokens: 12,
            completion_tokens: 4,
