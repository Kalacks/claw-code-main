...
#[allow(clippy::await_holding_lock)]
#[tokio::test]
async fn deepseek_streaming_requests_opt_into_usage_chunks() {
    let state = Arc::new(Mutex::new(Vec::<CapturedRequest>::new()));
    let sse = concat!(
        "data: {\"id\":\"chatcmpl_deepseek_stream\",\"model\":\"deepseek-chat\",\"choices\":[{\"delta\":{\"content\":\"Hi\"}}]}\n\n",
        "data: {\"id\":\"chatcmpl_deepseek_stream\",\"choices\":[{\"delta\":{},\"finish_reason\":\"stop\"}]}\n\n",
        "data: {\"id\":\"chatcmpl_deepseek_stream\",\"choices\":[],\"usage\":{\"prompt_tokens\":7,\"completion_tokens\":3,\"prompt_cache_hit_tokens\":2,\"prompt_cache_miss_tokens\":5}}\n\n",
        "data: [DONE]\n\n"
    );
    let server = spawn_server(
        state.clone(),
        vec![http_response_with_headers(
            ...
            "text/event-stream",
            sse,
            &[("x-request-id", "req_deepseek_stream")],
        )],
    )
    .await;
    ...
    let client = OpenAiCompatClient::new("deepseek-test-key", OpenAiCompatConfig::deepseek())
        .with_base_url(server.base_url());
    let mut stream = client
        .stream_message(&sample_request_for_model("deepseek-chat", true))
        .await
        .expect("stream should start");

    assert_eq!(stream.request_id(), Some("req_deepseek_stream"));
...
async fn provider_client_dispatches_deepseek_requests_from_env() {
    ...
    let server = spawn_server(
        ...
        vec![http_response(
            "200 OK",
            "application/json",
            "{\"id\":\"chatcmpl_provider\",\"model\":\"deepseek-chat\",\"choices\":[{\"message\":{\"role\":\"assistant\",\"content\":\"Through provider client\",\"tool_calls\":[]},\"finish_reason\":\"stop\"}],\"usage\":{\"prompt_tokens\":8,\"completion_tokens\":5,\"prompt_cache_hit_tokens\":3,\"prompt_cache_miss_tokens\":5}}",
        )],
    )
    .await;
    ...
    let response = client
        .send_message(&sample_request_for_model("deepseek-chat", false))
        .await
        .expect("provider-dispatched request should succeed");
...
#[allow(clippy::await_holding_lock)]
#[tokio::test]
async fn deepseek_request_history_omits_empty_tool_calls_on_assistant_text_messages() {
    let state = Arc::new(Mutex::new(Vec::<CapturedRequest>::new()));
    let server = spawn_server(
        state.clone(),
        vec![http_response(
            "200 OK",
            "application/json",
            "{\"id\":\"chatcmpl_history\",\"model\":\"deepseek-chat\",\"choices\":[{\"message\":{\"role\":\"assistant\",\"content\":\"ok\",\"tool_calls\":[]},\"finish_reason\":\"stop\"}],\"usage\":{\"prompt_tokens\":6,\"completion_tokens\":2}}",
        )],
    )
    .await;
    ...
    let request = MessageRequest {
        ...
        max_tokens: 64,
        messages: vec![
            InputMessage::user_text("1"),
            InputMessage {
                role: "assistant".to_string(),
                content: vec![InputContentBlock::Text {
                    text: "I see you've entered \"1\".".to_string(),
                }],
            },
            InputMessage::user_text("你现在在使用哪个api"),
        ],
        stream: false,
        ..Default::default()
    };
    ...
    let body: serde_json::Value =
        serde_json::from_str(&captured.first().expect("captured request").body).expect("json body");
    assert_eq!(body["messages"][0]["role"], json!("user"));
    assert_eq!(body["messages"][1]["role"], json!("assistant"));
    assert_eq!(
        body["messages"][1]["content"],
        json!("I see you've entered \"1\".")
    );
...
#[allow(clippy::await_holding_lock)]
#[tokio::test]
async fn deepseek_tool_messages_omit_is_error_and_empty_tool_config() {
    let state = Arc::new(Mutex::new(Vec::<CapturedRequest>::new()));
    let server = spawn_server(
        state.clone(),
        vec![http_response(
            "200 OK",
            "application/json",
            "{\"id\":\"chatcmpl_tool_result\",\"model\":\"deepseek-chat\",\"choices\":[{\"message\":{\"role\":\"assistant\",\"content\":\"ok\",\"tool_calls\":[]},\"finish_reason\":\"stop\"}],\"usage\":{\"prompt_tokens\":6,\"completion_tokens\":2}}",
        )],
    )
    .await;
    ...
    assert_eq!(body["messages"][0]["content"], json!("ok"));
...
fn sample_request(stream: bool) -> MessageRequest {
    sample_request_for_model("grok-3", stream)
}

fn sample_request_for_model(model: &str, stream: bool) -> MessageRequest {
    MessageRequest {
