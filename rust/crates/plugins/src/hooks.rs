...
mod tests {
    ...
    fn echo_and_exit_script(message: &str, exit_code: i32) -> String {
        if cfg!(windows) {
            format!("@echo off\r\necho {message}\r\nexit /b {exit_code}\r\n")
        } else {
            format!("#!/bin/sh\nprintf '%s\\n' '{message}'\nexit {exit_code}\n")
    ...
    fn write_hook_plugin(
        ...
        let failure_script = script_name("failure");
        // 测试脚本按平台生成，避免 Windows 直接执行 .sh 失败。
        write_script(
            root.join("hooks").join(&pre_script).as_path(),
            &echo_script(pre_message),
        );
        write_script(
            root.join("hooks").join(&post_script).as_path(),
            &echo_script(post_message),
        );
        write_script(
            root.join("hooks").join(&failure_script).as_path(),
            &echo_script(failure_message),
        );
        fs::write(
    ...
    fn pre_tool_use_denies_when_plugin_hook_exits_two() {
        ...
        let deny_script = root.join(script_name("deny"));
        write_script(&deny_script, &echo_and_exit_script("blocked by plugin", 2));
        let runner = HookRunner::new(crate::PluginHooks {
            pre_tool_use: vec![deny_script.display().to_string()],
    ...
    fn propagates_plugin_hook_failures() {
        ...
        fs::create_dir_all(&root).expect("temp dir");
        let failing_script = root.join(script_name("broken"));
        let later_script = root.join(script_name("later"));
        write_script(
            &failing_script,
            &echo_and_exit_script("broken plugin hook", 1),
        );
        write_script(&later_script, &echo_script("later plugin hook"));
        let runner = HookRunner::new(crate::PluginHooks {
            pre_tool_use: vec![
                failing_script.display().to_string(),
                later_script.display().to_string(),
            ],
            post_tool_use: Vec::new(),
            post_tool_use_failure: Vec::new(),
        });
