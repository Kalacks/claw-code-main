...
const DEFAULT_PLUGIN_TOOL_TIMEOUT_MS: u64 = 300_000;
...
impl PluginTool {
    ...
    pub fn execute(&self, input: &Value) -> Result<String, PluginError> {
        ...
        if let Some(reason) = detect_interactive_plugin_invocation(&self.command, &self.args, &cwd)
        {
            return Err(PluginError::CommandFailed(format!(
                "plugin tool `{}` from `{}` requires terminal handoff: {}",
                self.definition.name, self.plugin_id, reason
            )));
        }
        ...
        let output = loop {
            ...
            if started.elapsed() >= Duration::from_millis(DEFAULT_PLUGIN_TOOL_TIMEOUT_MS) {
                let _ = child.kill();
                let _ = child.wait_with_output();
                return Err(PluginError::CommandFailed(format!(
                    "plugin tool `{}` from `{}` timed out after {} ms",
                    self.definition.name, self.plugin_id, DEFAULT_PLUGIN_TOOL_TIMEOUT_MS
                )));
            }
...
fn detect_interactive_plugin_invocation(
    command: &str,
    args: &[String],
    cwd: &Path,
) -> Option<String> {
    let joined = std::iter::once(command)
        ...
        .collect::<Vec<_>>()
        .join(" ");
    let normalized = normalize_interactive_text(&joined);

    if normalized.contains(" pause ") || normalized.contains(" choice ") {
        return Some("detected Windows interactive command such as `pause` or `choice`".into());
    }

    if normalized.contains("read-host") || normalized.contains("-noexit") {
        return Some(
            "detected PowerShell interactive command such as `Read-Host` or `-NoExit`".into(),
        );
    }

    if normalized.contains(" read -p ") || normalized.contains(" select ") {
        return Some("detected shell interactive command such as `read -p`".into());
    }

    // 兼容 `cmd /C script.cmd`、`powershell -File script.ps1` 这类包装执行，
    // 避免只扫描最外层命令时漏掉真正的交互脚本。
    if let Some(reason) = detect_interactive_plugin_wrapper(command, args, cwd) {
        return Some(reason);
    }

    for path in referenced_plugin_script_paths(command, args, cwd) {
        if let Some(kind) = detect_interactive_plugin_script(&path) {
            return Some(format!(
                "detected interactive statement in script `{}`: {kind}",
                path.display()
            ));
        }
    }
...
fn detect_interactive_plugin_wrapper(command: &str, args: &[String], cwd: &Path) -> Option<String> {
    ...
    match command_name.as_str() {
        "cmd" | "cmd.exe" => detect_cmd_wrapper(args, cwd),
        "powershell" | "powershell.exe" | "pwsh" | "pwsh.exe" => {
            detect_powershell_wrapper(args, cwd)
        }
        "sh" | "bash" | "zsh" => detect_posix_shell_wrapper(args, cwd),
        _ => None,
    }
}

fn detect_cmd_wrapper(args: &[String], cwd: &Path) -> Option<String> {
    let wrapper_index = args
        .iter()
        .position(|arg| arg.eq_ignore_ascii_case("/c") || arg.eq_ignore_ascii_case("/k"))?;
    let nested = args.get(wrapper_index + 1..)?;
    let nested_command = nested.first()?;
    detect_interactive_plugin_invocation(nested_command, &nested[1..], cwd)
}

fn detect_powershell_wrapper(args: &[String], cwd: &Path) -> Option<String> {
    if let Some(index) = args
        .iter()
        .position(|arg| arg.eq_ignore_ascii_case("-file"))
    {
        let script = args.get(index + 1)?;
        return detect_interactive_plugin_invocation(script, &[], cwd);
    }
    ...
    {
        let nested = args.get(index + 1..)?;
        let nested_command = nested.join(" ");
        return detect_interactive_plugin_invocation(&nested_command, &[], cwd);
    }

    None
}

fn detect_posix_shell_wrapper(args: &[String], cwd: &Path) -> Option<String> {
    ...
    detect_interactive_plugin_invocation(script, &[], cwd)
}

fn normalize_interactive_text(text: &str) -> String {
    let lowered = text
        .to_ascii_lowercase()
        .replace('\r', "\n")
        .replace('\n', " ")
...
fn referenced_plugin_script_paths(command: &str, args: &[String], cwd: &Path) -> Vec<PathBuf> {
    std::iter::once(command.to_string())
        .chain(args.iter().cloned())
        .flat_map(|token| resolve_plugin_script_candidates(&token, cwd))
        .filter(|path| path.is_file())
        .collect()
}

fn resolve_plugin_script_candidates(token: &str, cwd: &Path) -> Vec<PathBuf> {
...
fn detect_interactive_plugin_script(path: &Path) -> Option<&'static str> {
    let contents = fs::read_to_string(path).ok()?;
    let normalized = normalize_interactive_text(&contents);
    if normalized.contains(" pause ") || normalized.contains(" choice ") {
...
mod tests {
    ...
    #[cfg(unix)]
    fn mark_script_executable(script_path: &Path) {
    ...
    #[cfg(not(unix))]
    fn mark_script_executable(_script_path: &Path) {}
    ...
    fn write_loader_plugin(root: &Path) {
        let pre_script = script_name("pre");
        let tool_script_name = script_name("echo-tool");
        let sync_script = script_name("sync");
        write_file(
            root.join("hooks").join(&pre_script).as_path(),
            echo_script("pre").as_str(),
        );
        write_file(
            root.join("tools").join(&tool_script_name).as_path(),
            tool_script().as_str(),
        );
        write_file(
            root.join("commands").join(&sync_script).as_path(),
            echo_script("sync").as_str(),
        );
        write_file(
            root.join(MANIFEST_FILE_NAME).as_path(),
            format!(
                "{{\n  \"name\": \"loader-demo\",\n  \"version\": \"1.2.3\",\n  \"description\": \"Manifest loader test plugin\",\n  \"permissions\": [\"read\", \"write\"],\n  \"hooks\": {{\n    \"PreToolUse\": [\"./hooks/{pre_script}\"]\n  }},\n  \"tools\": [\n    {{\n      \"name\": \"echo_tool\",\n      \"description\": \"Echoes JSON input\",\n      \"inputSchema\": {{\n        \"type\": \"object\"\n      }},\n      \"command\": \"./tools/{tool_script_name}\",\n      \"requiredPermission\": \"workspace-write\"\n    }}\n  ],\n  \"commands\": [\n    {{\n      \"name\": \"sync\",\n      \"description\": \"Sync command\",\n      \"command\": \"./commands/{sync_script}\"\n    }}\n  ]\n}}"
            )
            .as_str(),
        );
    }
    ...
    fn write_external_plugin(root: &Path, name: &str, version: &str) {
        let pre_script = script_name("pre");
        let post_script = script_name("post");
        write_file(
            root.join("hooks").join(&pre_script).as_path(),
            echo_script("pre").as_str(),
        );
        write_file(
            root.join("hooks").join(&post_script).as_path(),
            echo_script("post").as_str(),
        );
        write_file(
    ...
    fn interactive_tool_script() -> String {
        if cfg!(windows) {
            "@echo off\r\necho before\r\npause\r\n".to_string()
        } else {
    ...
    fn write_tool_plugin_with_name(root: &Path, name: &str, version: &str, tool_name: &str) {
        let tool_runner = if cfg!(windows) { "cmd" } else { "sh" };
        let tool_args = if cfg!(windows) {
            r#"["/C", ".\\tools\\echo-json.cmd"]"#
        } else {
            r#"["./tools/echo-json.sh"]"#
        };
        let script_path = root.join("tools").join(script_name("echo-json"));
        write_script(&script_path, tool_script().as_str());
        write_file(
            root.join(MANIFEST_RELATIVE_PATH).as_path(),
            format!(
    ...
    fn write_interactive_tool_plugin(root: &Path, name: &str, version: &str) {
        let tool_runner = if cfg!(windows) { "cmd" } else { "sh" };
        let tool_args = if cfg!(windows) {
            r#"["/C", ".\\tools\\interactive.cmd"]"#
        } else {
            r#"["./tools/interactive.sh"]"#
        };
        let script_path = root.join("tools").join(script_name("interactive"));
        write_script(&script_path, interactive_tool_script().as_str());
        write_file(
            root.join(MANIFEST_RELATIVE_PATH).as_path(),
            format!(
                "{{\n  \"name\": \"{name}\",\n  \"version\": \"{version}\",\n  \"description\": \"interactive tool plugin\",\n  \"tools\": [\n    {{\n      \"name\": \"interactive_tool\",\n      \"description\": \"Interactive JSON input\",\n      \"inputSchema\": {{\"type\": \"object\"}},\n      \"command\": \"{tool_runner}\",\n      \"args\": {tool_args},\n      \"requiredPermission\": \"workspace-write\"\n    }}\n  ]\n}}"
            )
            .as_str(),
        );
    }
    ...
    fn load_plugin_from_directory_reads_root_manifest_and_validates_entries() {
        ...
        assert_eq!(
            ...
            vec![format!("./hooks/{}", script_name("pre"))]
        );
  ...
  "commands": [
    {{"name": "sync", "description": "Sync one", "command": "./commands/{sync_script}"}},
    {{"name": "sync", "description": "Sync two", "command": "./commands/{sync_script}"}}
  ]
}}"#
            )
            .as_str(),
            ...
            PluginError::ManifestValidation(errors) => {
                assert!(errors.iter().any(|error| matches!(
                    ...
                    if *kind == "lifecycle command"
                        && path.ends_with(Path::new(&format!("lifecycle/{init_script}")))
                )));
                assert!(errors.iter().any(|error| matches!(
                    error,
                    PluginManifestValidationError::MissingPath { kind, path }
                    if *kind == "lifecycle command"
                        && path.ends_with(Path::new(&format!("lifecycle/{shutdown_script}")))
                )));
            }
            other => panic!("expected manifest validation errors, got {other}"),
        }
  ...
  "commands": [
    ...
    fn installs_enables_updates_and_uninstalls_external_plugins() {
        ...
        assert!(hooks.pre_tool_use[0].contains(script_name("pre").as_str()));
  ...
  "description": "Registry fallback plugin"
    ...
    #[test]
    fn plugin_tools_require_terminal_handoff_for_interactive_scripts() {
        let config_home = temp_dir("interactive-tool-home");
        let source_root = temp_dir("interactive-tool-source");
        write_interactive_tool_plugin(&source_root, "interactive-demo", "1.0.0");
        ...
        let error = tools[0]
            ...
            .expect_err("interactive plugin tool should be blocked");
        assert!(error.to_string().contains("terminal handoff"));
