// Modified to force rebuild at 2026-04-10 22:50
#![allow(
    dead_code,
    unused_imports,
    unused_variables,
...
mod agent_layer;
mod cli_runtime_actions;
mod controller_layer;
mod gui_layer;
mod init;
mod input;
mod llm_layer;
...
use agent_layer::{AgentLayerSummary, AgentWorkspaceStore, AppRecord, ThreadRecord};
use commands::{
...
use controller_layer::{
    execute_app_controller_action, execute_llm_controller_action, execute_thread_controller_action,
    format_app_removed_report, format_app_saved_report, format_apps_report,
    format_llm_limits_report, format_llm_limits_updated_report,
    format_llm_profile_activated_report, format_llm_profile_cleared_report,
    format_llm_profile_removed_report, format_llm_profile_saved_report, format_llm_profiles_report,
    format_thread_removed_report, format_thread_saved_report, format_thread_switched_report,
    format_threads_report, parse_app_command, parse_llm_command, parse_thread_command,
    resolve_workspace_path, sanitize_for_session_id as controller_sanitize_for_session_id,
    AppCommandAction, AppCommandEffect, AppRunTarget, ControllerDispatch, LlmCommandAction,
    LlmCommandEffect, LlmLimitsAction, ThreadCommandAction, ThreadCommandEffect,
};
use init::initialize_repo;
use llm_layer::{
    estimate_text_tokens, LlmLayerSummary, LlmProfile, LlmProfileStore, TurnTokenLimits,
};
use plugins::{PluginHooks, PluginManager, PluginManagerConfig, PluginRegistry};
use render::{MarkdownStreamState, Spinner, TerminalRenderer};
use runtime::{
    check_base_commit, clear_oauth_credentials, format_currency, format_stale_base_warning,
    generate_pkce_pair, generate_state, load_oauth_credentials, load_system_prompt,
    parse_oauth_callback_request_target, pricing_for_usage, resolve_expected_base,
    resolve_pricing_for_model, resolve_sandbox_status, save_oauth_credentials, ApiClient,
...
use ui_layer::{
    render_app_help, render_client_layer_help, render_client_workspace_overview, render_llm_help,
    render_startup_dashboard, render_thread_help, render_workspace_dashboard, ClientSurface,
    DashboardUsage,
};
...
fn resolve_repl_model(cli_model: String) -> String {
    ...
    if let Some(env_model) = first_non_empty_env_model(&[
        "CLAW_MODEL",
        "ANTHROPIC_MODEL",
        "OPENAI_MODEL",
        "XAI_MODEL",
...
fn first_non_empty_env_model(keys: &[&str]) -> Option<String> {
...
fn provider_default_base_url(provider: &str) -> Option<String> {
    match provider {
        "anthropic" => Some(api::read_base_url()),
        "xai" => Some(api::read_xai_base_url()),
        "deepseek" => Some(api::read_deepseek_base_url()),
        "openai" | "compat" => Some(
...
fn connection_details(model: &str, profile: Option<&LlmProfile>) -> (String, String) {
    ...
    let provider = provider_label(detect_provider_kind(model)).to_string();
    let base_url =
        provider_default_base_url(&provider).unwrap_or_else(|| "(provider default)".to_string());
    (provider, base_url)
}
...
fn validate_llm_profile_input(
    ...
    controller_layer::validate_llm_profile_input(name, provider, model, api_key_env, base_url)
}
...
fn render_doctor_report() -> Result<DoctorReport, Box<dyn std::error::Error>> {
    ...
    let llm_summary = LlmProfileStore::load_for(&cwd)
        .map(|store| store.summary())
        .unwrap_or(LlmLayerSummary {
    ...
    let agent_summary = AgentWorkspaceStore::load_for(&cwd)
        .map(|store| store.summary())
        .unwrap_or(AgentLayerSummary {
...
struct StatusContext {
    ...
    agent_summary: AgentLayerSummary,
}
...
fn estimated_cost_display(model: Option<&str>, usage: TokenUsage) -> Option<String> {
    let pricing = model
        .and_then(|model_name| pricing_for_usage(model_name, usage))
        .or_else(|| resolve_pricing_for_model(model).pricing);
    pricing.map(|pricing| {
        let estimate = usage.estimate_cost_with_pricing(pricing);
        format_currency(estimate.total_cost(), estimate.currency)
    })
}

fn format_cost_report(model: Option<&str>, usage: TokenUsage) -> String {
    let pricing = resolve_pricing_for_model(model);
    let estimated_cost =
        estimated_cost_display(model, usage).unwrap_or_else(|| "unavailable".to_string());
    format!(
        "Cost
  Input tokens     {}
  Output tokens    {}
...
fn format_turn_usage_report(model: &str, latest: TokenUsage, cumulative: TokenUsage) -> String {
    let pricing = resolve_pricing_for_model(Some(model));
    let latest_cost =
        estimated_cost_display(Some(model), latest).unwrap_or_else(|| "unavailable".to_string());
    let cumulative_cost = estimated_cost_display(Some(model), cumulative)
...
fn run_resume_command(
    ...
    match command {
        ...
        SlashCommand::Cost => {
            ...
            Ok(ResumeCommandOutcome {
                session: session.clone(),
                message: Some(format_cost_report(None, usage)),
                json: None,
            })
        }
    ...
    loop {
        ...
        match editor.read_line()? {
            input::ReadOutcome::Submit(input) => {
                let submissions = split_repl_submission(&input);
                if submissions.is_empty() {
                    continue;
                }

                let should_store_raw_history = submissions.len() == 1 && !input.contains('\n');
                if should_store_raw_history {
                    editor.push_history(input);
                }

                for submission in submissions {
                    match submission {
                        ReplSubmission::Slash(command_text) => {
                            if matches!(command_text.as_str(), "/exit" | "/quit") {
                                cli.persist_session()?;
                                return Ok(());
                            }
                            match SlashCommand::parse(&command_text) {
                                Ok(Some(command)) => {
                        ...
                        ReplSubmission::Prompt(prompt) => {
                            if !should_store_raw_history {
                                editor.push_history(prompt.clone());
                            }
                            cli.record_prompt_history(prompt.trim());
                            cli.run_turn(&prompt)?;
                        }
                    }
                }
...
fn split_repl_submission(input: &str) -> Vec<ReplSubmission> {
    ...
    if !lines.is_empty() && lines.iter().all(|line| line.starts_with('/')) {
        return lines
            .into_iter()
            .map(|line| ReplSubmission::Slash(line.to_string()))
            .collect();
    }

    vec![ReplSubmission::Prompt(trimmed.to_string())]
}
...
struct LiveCli {
    ...
    thread_store: AgentWorkspaceStore,
...
impl LiveCli {
    fn new(
        ...
        let cwd = env::current_dir()?;
        let llm_store = LlmProfileStore::load_for(&cwd)?;
        let thread_store = AgentWorkspaceStore::load_for(&cwd)?;
        let mut active_model = model;
        if let Some(profile) = llm_store.active_profile() {
            active_model = resolve_model_alias_with_config(&profile.model);
        }
        let system_prompt = build_system_prompt(&active_model, llm_store.active_profile())?;
        let turn_token_limits = llm_store.turn_token_limits().validate()?;
        ...
        let runtime = build_runtime(
            ...
            permission_mode,
            None,
            llm_store.active_profile().cloned(),
            turn_token_limits,
        )?;
        let cli = Self {
            model: active_model,
    ...
    fn startup_banner(&self) -> String {
        if let Ok(context) = status_context(Some(&self.session.path)) {
            return render_startup_dashboard(&self.client_surface_from_context(&context));
        }
 ...
 ╚═════╝╚══════╝╚═╝  ╚═╝ ╚══╝╚══╝\x1b[0m \x1b[38;5;208mCode\x1b[0m 🦞\n\n\
  ...
  Type \x1b[1m/help\x1b[0m for commands · \x1b[1m/status\x1b[0m for live context · \x1b[2m/resume latest\x1b[0m jumps back to the newest session · \x1b[1m/diff\x1b[0m then \x1b[1m/commit\x1b[0m to ship · \x1b[2mTab\x1b[0m for workflow completions · \x1b[2mShift+Enter\x1b[0m for newline",
    ...
    fn connected_line(&self) -> String {
        if let Some(profile) = self.llm_store.active_profile() {
            return format!(
                "Connected: {} via {} ({})",
                self.model,
        ...
        format_connected_line(&self.model)
    }

    fn active_llm_profile(&self) -> Option<LlmProfile> {
        self.llm_store.active_profile().cloned()
    }

    fn client_surface_from_context(&self, context: &StatusContext) -> ClientSurface {
        let session_path = self.session.path.strip_prefix(&context.cwd).map_or_else(
            |_| self.session.path.display().to_string(),
            |path| path.display().to_string(),
        );
        let (provider, base_url) = connection_details(&self.model, self.llm_store.active_profile());
        ...
        ClientSurface {
            ...
            workspace_state: context.git_summary.headline(),
            directory: context.cwd.display().to_string(),
            session_id: self.session.id.clone(),
            session_path,
            llm: context.llm_summary.clone(),
            agent: context.agent_summary.clone(),
        }
    }

    fn dashboard_usage(&self) -> DashboardUsage {
        let pricing = resolve_pricing_for_model(Some(&self.model));
        ...
        DashboardUsage {
            ...
            session_total_tokens: cumulative.total_tokens(),
            currency: pricing.currency.symbol().to_string(),
            pricing: pricing.source.as_str().to_string(),
            latest_cost: estimated_cost_display(Some(&self.model), latest)
                .unwrap_or_else(|| "unavailable".to_string()),
            session_cost: estimated_cost_display(Some(&self.model), cumulative)
                .unwrap_or_else(|| "unavailable".to_string()),
        }
    }
    ...
    fn validate_turn_input_limits(&self, input: &str) -> Result<(), Box<dyn std::error::Error>> {
        let estimated_input_tokens = estimate_text_tokens(input);
        self.turn_token_limits
            .check_input_estimate(estimated_input_tokens)
            .map_err(|error| -> Box<dyn std::error::Error> { error.into() })
    }

    fn maybe_print_turn_output_limit_warning(&self, output_tokens: u32) {
        if let Some(warning) = self.turn_token_limits.output_limit_warning(output_tokens) {
            eprintln!("token-limits: {warning}");
        }
    }

    fn run_turn(&mut self, input: &str) -> Result<(), Box<dyn std::error::Error>> {
        self.validate_turn_input_limits(input)?;
        ...
        match result {
            Ok(summary) => {
                ...
                self.maybe_print_turn_output_limit_warning(summary.usage.output_tokens);
                println!(
                    "{}",
                    format_turn_usage_report(
                        &self.model,
    ...
    fn run_prompt_json(&mut self, input: &str) -> Result<(), Box<dyn std::error::Error>> {
        ...
        println!(
            ...
            json!({
                ...
                "cost_currency": pricing.currency.symbol(),
                "pricing_source": pricing.source.as_str(),
                "estimated_cost": estimated_cost_display(Some(&self.model), summary.usage)
            })
        );
        Ok(())
    }
    ...
    fn handle_repl_command(
        ...
        Ok(match command {
            ...
            SlashCommand::Llm { args } => {
                self.handle_llm_command(args.as_deref())?;
                false
            }
            SlashCommand::Thread { args } => {
                self.handle_thread_command(args.as_deref())?;
                false
            }
            SlashCommand::App { args } => {
                self.handle_app_command(args.as_deref())?;
                false
            }
            SlashCommand::Workspace { path } => self.handle_workspace_command(path)?,
            SlashCommand::Doctor => {
    ...
    fn print_workspace_dashboard(&self) -> Result<(), Box<dyn std::error::Error>> {
        let context = status_context(Some(&self.session.path))?;
        let surface = self.client_surface_from_context(&context);
        let usage = self.dashboard_usage();
        println!("{}", render_workspace_dashboard(&surface, Some(&usage)));
        Ok(())
    }

    fn reload_workspace_state(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        let cwd = env::current_dir()?;
        self.llm_store = LlmProfileStore::load_for(&cwd)?;
        self.thread_store = AgentWorkspaceStore::load_for(&cwd)?;
        self.turn_token_limits = self.llm_store.turn_token_limits().validate()?;

        let next_model = self
            .llm_store
            .active_profile()
            .map(|profile| resolve_model_alias_with_config(&profile.model))
            .unwrap_or_else(|| self.model.clone());
        self.system_prompt = build_system_prompt(&next_model, self.llm_store.active_profile())?;
    ...
    fn handle_workspace_command(
        ...
        let Some(raw_path) = path
            ...
            self.print_workspace_dashboard()?;
            return Ok(false);
        };

        let previous = env::current_dir()?;
        let next = resolve_thread_folder(&raw_path)?;
        if !next.exists() {
            return Err(format!("workspace directory does not exist: {}", next.display()).into());
        }
        if !next.is_dir() {
            return Err(format!("workspace path is not a directory: {}", next.display()).into());
        }

        env::set_current_dir(&next)?;
        self.reload_workspace_state()?;
        println!(
            "Workspace switched\n  Previous         {}\n  Current          {}",
            previous.display(),
            next.display(),
    ...
    fn print_cost(&self) {
        let cumulative = self.runtime.usage().cumulative_usage();
        println!("{}", format_cost_report(Some(&self.model), cumulative));
    }
    ...
    fn handle_llm_command(&mut self, args: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
        match parse_llm_command(
            args,
            self.llm_store.active_profile_name(),
            self.turn_token_limits,
        ) {
            ControllerDispatch::Message(message) => println!("{message}"),
            ControllerDispatch::Action(action) => {
                match execute_llm_controller_action(
                    &mut self.llm_store,
                    action,
                    self.turn_token_limits,
                )? {
                    ControllerDispatch::Message(message) => println!("{message}"),
                    ControllerDispatch::Action(effect) => match effect {
                        LlmCommandEffect::Activate { profile } => {
                            self.model = resolve_model_alias_with_config(&profile.model);
                            self.reload_runtime_features()?;
                            println!(
                                "{}",
                                format_llm_profile_activated_report(
                                    &profile,
                                    &self.model,
                                    &self.connected_line()
                                )
                        ...
                        LlmCommandEffect::ClearActive => {
                            self.reload_runtime_features()?;
                            println!("{}", format_llm_profile_cleared_report());
                        }
                        LlmCommandEffect::UpdateLimits { label, limits } => {
                            self.turn_token_limits = limits;
                            self.reload_runtime_features()?;
                            println!("{}", format_llm_limits_updated_report(label, limits));
                        }
                    },
                }
            }
    ...
    fn handle_thread_command(
        ...
        cli_runtime_actions::handle_thread_command(self, args)
    }

    fn handle_app_command(&mut self, args: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
        cli_runtime_actions::handle_app_command(self, args)
    }
...
fn sessions_dir_for(cwd: &Path) -> Result<PathBuf, Box<dyn std::error::Error>> {
    let store = runtime::SessionStore::from_cwd(cwd)
        .map_err(|e| Box::new(e) as Box<dyn std::error::Error>)?;
    Ok(store.sessions_dir().to_path_buf())
}
...
fn resolve_thread_folder(raw: &str) -> Result<PathBuf, Box<dyn std::error::Error>> {
    Ok(resolve_workspace_path(&env::current_dir()?, raw))
}

fn sanitize_for_session_id(name: &str) -> String {
    controller_sanitize_for_session_id(name)
}
...
fn status_json_value(
    ...
    json!({
        ...
        "usage": {
            ...
            "latest_estimated_cost": estimated_cost_display(Some(model), usage.latest),
            "cumulative_estimated_cost": estimated_cost_display(Some(model), usage.cumulative),
            "cost_currency": pricing.currency.symbol(),
            "pricing_source": pricing.source.as_str(),
        },
        "layers": {
            "llm": {
                "active_profile": context.llm_summary.active_profile_name.clone(),
                "profiles": context.llm_summary.profile_count,
                "turn_limits": context.llm_summary.turn_token_limits.summary_line(),
            },
            "agent": {
                "active_thread": context.agent_summary.active_thread_name.clone(),
                "threads": context.agent_summary.thread_count,
                "apps": context.agent_summary.app_count,
                "skills_entrypoint": "/skills list",
            }
        },
        "workspace": {
            "cwd": context.cwd,
...
fn status_context(
    ...
    let runtime_config = loader.load()?;
    let llm_summary = LlmProfileStore::load_for(&cwd)?.summary();
    let agent_summary = AgentWorkspaceStore::load_for(&cwd)?.summary();
    let project_context = ProjectContext::discover_with_git(&cwd, DEFAULT_DATE)?;
    let (project_root, git_branch) =
        parse_git_status_metadata(project_context.git_status.as_deref());
    let git_summary = parse_git_workspace_summary(project_context.git_status.as_deref());
...
fn format_status_report(
    ...
    let base_url = context
        ...
        .or_else(|| provider_default_base_url(&provider))
        .unwrap_or_else(|| "(provider default)".to_string());
    let latest_cost = estimated_cost_display(Some(model), usage.latest)
        .unwrap_or_else(|| "unavailable".to_string());
    let cumulative_cost = estimated_cost_display(Some(model), usage.cumulative)
        .unwrap_or_else(|| "unavailable".to_string());
    [
        format!(
            "Status
  ...
  Session cost     {}",
        ...
        render_client_workspace_overview(&context.llm_summary, &context.agent_summary),
        format!(
            "Workspace
  Cwd              {}
  Project root     {}
...
fn provider_client_from_llm_profile(
    profile: &LlmProfile,
    ...
    let client = match provider.as_str() {
        ...
        other => {
            return Err(format!(
                "unsupported llm provider '{}' for profile '{}'; use anthropic|deepseek|openai|xai|compat",
                other, profile.name
            )
            .into());
        }
...
impl ApiClient for ProviderRuntimeClient {
    ...
    fn stream(&mut self, request: ApiRequest) -> Result<Vec<AssistantEvent>, RuntimeError> {
        ...
        let model_default_max_tokens = max_tokens_for_model(&self.model);
...
fn format_tool_result(name: &str, output: &str, is_error: bool) -> String {
    ...
    match name {
        "bash" | "Bash" => format_shell_result(icon, "bash", &parsed),
        "PowerShell" => format_shell_result(icon, "PowerShell", &parsed),
...
fn format_shell_result(icon: &str, label: &str, parsed: &serde_json::Value) -> String {
    ...
    {
        write!(&mut lines[0], " {}", format_shell_status(status)).expect("write to string");
    }
...
fn format_shell_status(status: &str) -> String {
    match status {
        "interactive_blocked" => "interactive blocked; run in terminal".to_string(),
...
mod tests {
    use super::{
        append_connection_prompt_details, build_runtime_plugin_state_with_loader,
        build_runtime_with_plugin_state, collect_session_prompt_history,
        create_managed_session_handle, describe_tool_progress, filter_tool_specs,
        format_apps_report, format_bughunter_report, format_commit_preflight_report,
        ...
        format_issue_report, format_llm_profiles_report, format_model_report,
        format_model_switch_report, format_permissions_report, format_permissions_switch_report,
        format_pr_report, format_resume_report, format_status_report, format_threads_report,
        format_tool_call_start, format_tool_result, format_turn_usage_report,
        ...
        slash_command_completion_candidates_with_sessions, split_repl_submission, status_context,
        summarize_tool_payload_for_markdown, validate_llm_profile_input, validate_no_args,
        write_mcp_server_fixture, AgentLayerSummary, CliAction, CliOutputFormat, CliToolExecutor,
        GitWorkspaceSummary, InternalPromptProgressEvent, InternalPromptProgressState, LiveCli,
        LlmLayerSummary, LocalHelpTopic, PromptHistoryEntry, ReplSubmission, SlashCommand,
        StatusUsage, TurnTokenLimits, DEFAULT_MODEL, LATEST_SESSION_REFERENCE,
    };
    use crate::llm_layer::LlmProfile;
    ...
    /// 将测试临时目录固定到当前盘符根目录，避免 `%TEMP%` 落在用户主目录 Git worktree
    /// 或磁盘空间不足的分区上，导致 Git/会话/MCP 相关测试在 Windows 上误判。
    fn test_temp_root() -> PathBuf {
        ...
        drive_root.join("claw-cli-test-tmp")
    }
    ...
    fn temp_dir() -> PathBuf {
        ...
        test_temp_root().join(format!("rusty-claude-cli-{nanos}-{unique}"))
    }
    ...
    fn temp_workspace(label: &str) -> PathBuf {
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        test_temp_root().join(format!("claw-cli-{label}-{nanos}"))
    }
    ...
    fn write_script(path: &Path, contents: &str) {
        let parent = path.parent().expect("script path should have parent");
    ...
    fn echo_script(message: &str) -> String {
        if cfg!(windows) {
            format!("@echo off\r\necho {message}\r\n")
        } else {
            format!("#!/bin/sh\nprintf '%s\\n' '{message}'\n")
    ...
    fn lifecycle_script(message: &str) -> String {
        if cfg!(windows) {
            format!("@echo off\r\necho {message}>> lifecycle.log\r\n")
        } else {
            format!("#!/bin/sh\nprintf '{message}\\n' >> lifecycle.log\n")
        }
    }
    ...
    fn remove_dir_all_with_retry(path: &Path) {
        ...
        panic!(
            "cleanup should succeed for {}: {}",
            path.display(),
            last_error.expect("cleanup error should exist")
        );
    }

    fn write_plugin_fixture(root: &Path, name: &str, include_hooks: bool, include_lifecycle: bool) {
        fs::create_dir_all(root.join(".claude-plugin")).expect("manifest dir");
        let hook_script = script_name("pre");
        let init_script = script_name("init");
        let shutdown_script = script_name("shutdown");
        if include_hooks {
            write_script(
                root.join("hooks").join(&hook_script).as_path(),
                echo_script("plugin pre hook").as_str(),
            );
        }
        if include_lifecycle {
            write_script(
                root.join("lifecycle").join(&init_script).as_path(),
                lifecycle_script("init").as_str(),
            );
            write_script(
                root.join("lifecycle").join(&shutdown_script).as_path(),
                lifecycle_script("shutdown").as_str(),
            );
        }
    ...
    fn repl_help_includes_shared_commands_and_exit() {
        ...
        assert!(help.contains("Client Layers"));
        assert!(help.contains("/llm import"));
        assert!(help.contains("/thread add"));
    ...
    fn format_connected_line_renders_deepseek_provider_for_deepseek_model() {
        ...
        assert_eq!(line, "Connected: deepseek-chat via deepseek");
    }

    #[test]
    fn validate_llm_profile_input_rejects_non_url_base_url() {
        let error = validate_llm_profile_input(
            "deepseek-main",
            "deepseek",
            "deepseek-chat",
        ...
        assert!(error.contains("base-url must start with http:// or https://"));
    }

    #[test]
    fn connection_prompt_details_pin_the_active_provider_identity() {
        let mut prompt = vec!["# System".to_string()];
        ...
        append_connection_prompt_details(&mut prompt, "deepseek-chat", Some(&profile));

        let rendered = prompt.join("\n\n");
        assert!(rendered.contains("Connected provider: deepseek."));
        assert!(rendered.contains("Connected model: deepseek-chat."));
        assert!(rendered
            .contains("do not claim to be Claude unless the connected provider is Anthropic"));
    }

    #[test]
    fn split_repl_submission_breaks_multiline_slash_paste_into_commands() {
        let submissions = split_repl_submission("/status\n/llm list\n/workspace");

        assert_eq!(
            submissions,
            vec![
                ReplSubmission::Slash("/status".to_string()),
                ReplSubmission::Slash("/llm list".to_string()),
                ReplSubmission::Slash("/workspace".to_string()),
            ]
        );
    }

    #[test]
    fn split_repl_submission_preserves_multiline_prompt_text() {
        let submissions = split_repl_submission("第一行\n第二行\n第三行");

        assert_eq!(
            submissions,
            vec![ReplSubmission::Prompt("第一行\n第二行\n第三行".to_string())]
        );
    }
    ...
    #[test]
    fn resolve_repl_model_prefers_generic_model_env_for_deepseek() {
        let _guard = env_lock();
        let root = temp_dir();
        fs::create_dir_all(&root).expect("root dir");
        let config_home = root.join("config");
        fs::create_dir_all(&config_home).expect("config home dir");
        std::env::set_var("CLAW_CONFIG_HOME", &config_home);
        std::env::set_var("CLAW_MODEL", "deepseek");
        std::env::remove_var("ANTHROPIC_MODEL");
    ...
    fn cost_report_uses_sectioned_layout() {
        ...
        assert!(report.contains("Currency         ¥"));
        assert!(report.contains("Pricing          official"));
        assert!(report.contains("Est. cost        ¥0.0001"));
    }

    #[test]
    fn cost_report_marks_unpriced_chinese_models_as_rmb_unavailable() {
        let report = format_cost_report(
            Some("doubao-pro"),
            runtime::TokenUsage {
                input_tokens: 20,
        ...
        assert!(report.contains("Currency         ¥"));
        assert!(report.contains("Pricing          unavailable"));
        assert!(report.contains("Est. cost        unavailable"));
    }

    #[test]
    fn cost_report_uses_official_kimi_pricing() {
        let report = format_cost_report(
            Some("kimi-k2.5"),
            runtime::TokenUsage {
                input_tokens: 1_000,
        ...
        assert!(report.contains("Currency         ¥"));
        assert!(report.contains("Pricing          official"));
        assert!(report.contains("Est. cost        ¥0.0146"));
    }

    #[test]
    fn turn_usage_report_surfaces_turn_and_session_totals() {
        let report = format_turn_usage_report(
            "deepseek-chat",
            runtime::TokenUsage {
                input_tokens: 20,
                output_tokens: 8,
        ...
        assert!(report.contains("Turn input       20"));
        assert!(report.contains("Turn output      8"));
        assert!(report.contains("Turn cache write 3"));
        assert!(report.contains("Turn cache read  1"));
        assert!(report.contains("Turn total       32"));
        assert!(report.contains("Session total    184"));
        assert!(report.contains("Currency         ¥"));
        assert!(report.contains("Pricing          official"));
        assert!(report.contains("Est. turn cost"));
        assert!(report.contains("Est. session cost"));
        assert!(report.contains("¥0.0001"));
        assert!(report.contains("¥0.0004"));
    }
    ...
    fn status_line_reports_model_and_token_totals() {
        let status = format_status_report(
            ...
            &super::StatusContext {
                ...
                agent_summary: AgentLayerSummary {
                    active_thread_name: Some("backend".to_string()),
                    thread_count: 3,
                    app_count: 2,
                },
            },
        ...
        assert!(status.contains("Provider         deepseek"));
        assert!(status.contains("Base URL         https://api.deepseek.com"));
        assert!(status.contains("Permission mode  workspace-write"));
        assert!(status.contains("Messages         7"));
        assert!(status.contains("Estimated tokens 128"));
        assert!(status.contains("Latest total     10"));
        assert!(status.contains("Cumulative total 31"));
        assert!(status.contains("Cost"));
        assert!(status.contains("Workspace Layers"));
        assert!(status.contains("LLM profile      deepseek-main"));
        assert!(status.contains("Saved profiles   2"));
        assert!(status.contains("Active thread    backend"));
        assert!(status.contains("Saved threads    3"));
        assert!(status.contains("Saved apps       2"));
    ...
    #[test]
    fn llm_profiles_report_uses_single_empty_state_line() {
        let report = format_llm_profiles_report(
            Path::new("/tmp/project/.claw/llm-layer.json"),
            None,
        ...
        assert!(report.contains("Empty state      No saved LLM profiles yet."));
        assert!(!report.contains("Profiles         (none)"));
    }

    #[test]
    fn threads_report_uses_single_empty_state_line() {
        let report =
            format_threads_report(Path::new("/tmp/project/.claw/agent-layer.json"), None, &[]);

        assert!(report.contains("Threads          0"));
        assert!(report.contains("Empty state      No saved threads yet."));
        assert!(!report.contains("Threads          (none)"));
    }

    #[test]
    fn apps_report_uses_single_empty_state_line() {
        let report = format_apps_report(Path::new("/tmp/project/.claw/agent-layer.json"), &[]);

        assert!(report.contains("Apps             0"));
        assert!(report.contains("Empty state      No saved apps yet."));
        assert!(report.contains("Built-in tips    /skills list"));
        assert!(!report.contains("Apps             (none)"));
    }
    ...
    fn config_report_supports_section_views() {
        let _guard = env_lock();
        let workspace = temp_workspace("config-report-sections");
        fs::create_dir_all(&workspace).expect("workspace should create");

        let report = with_current_dir(&workspace, || render_config_report(Some("env")))
            .expect("config report should render");
        assert!(report.contains("Merged section: env"));
        let plugins_report = with_current_dir(&workspace, || render_config_report(Some("plugins")))
    ...
    fn config_report_uses_sectioned_layout() {
        let _guard = env_lock();
        let workspace = temp_workspace("config-report-layout");
        fs::create_dir_all(&workspace).expect("workspace should create");

        let report = with_current_dir(&workspace, || render_config_report(None))
            .expect("config report should render");
...
UU conflicted.rs",
    ...
    fn managed_sessions_default_to_jsonl_and_resolve_legacy_json() {
        ...
        let legacy_path = handle
            .path
            .parent()
            .expect("managed session should have parent directory")
            .join("legacy.json");
    ...
    fn build_runtime_plugin_state_merges_plugin_hooks_into_runtime_features() {
        ...
        assert!(
            pre_hooks[0].ends_with(format!("hooks/{}", script_name("pre")).as_str()),
    ...
    fn build_runtime_runs_plugin_lifecycle_init_and_shutdown() {
        let _guard = env_lock();
        let config_home = temp_dir();
        // The default model is deepseek-chat, so this test injects a matching dummy key.
        let original_deepseek_api_key = std::env::var("DEEPSEEK_API_KEY").ok();
        ...
        let _ = fs::remove_dir_all(workspace);
        let _ = fs::remove_dir_all(source_root);
        match original_deepseek_api_key {
