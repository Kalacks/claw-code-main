...
OpenAI-compatible providers can be configured the same way. For DeepSeek:
...
# Chat model
cargo run -p rusty-claude-cli -- --model deepseek-chat

# Built-in alias for deepseek-chat
cargo run -p rusty-claude-cli -- --model deepseek
```
